...
class Hand:
    ...
    def get_sorted_card_number_representations(self):
        return sorted([card.number_representation for card in self.cards], reverse=True)

    def score(self):
        if self.is_royal_flush():
            return 10
        if self.is_straight_flush():
            return 9, self.get_sorted_card_number_representations()
        four_of_a_kind = self.is_four_of_a_kind()
        if not isinstance(four_of_a_kind, Boolean) and four_of_a_kind[0]:
            return 8, four_of_a_kind[1], four_of_a_kind[2]
        full_house = self.is_full_house()
        if not isinstance(full_house, Boolean) and full_house[0]:
            return 7, full_house[1], full_house[2]
        if self.is_flush():
            return 6, self.get_sorted_card_number_representations()
        if self.is_straight():
            return 5, self.get_sorted_card_number_representations()
        three_of_a_kind = self.is_three_of_a_kind()
        if not isinstance(three_of_a_kind, Boolean) and three_of_a_kind[0]:
            return 4, three_of_a_kind[1], three_of_a_kind[2]
        two_pairs = self.is_two_pairs()
        if not isinstance(two_pairs, Boolean) and two_pairs[0]:
            return 3, two_pairs[1], two_pairs[2], two_pairs[3]
        pair = self.is_pair()
        if not isinstance(pair, Boolean) and pair[0]:
            return 2, pair[1], pair[2]
        return 1, self.get_sorted_card_number_representations()

    def is_royal_flush(self):
        return all([self.cards[0].suit == card.suit for card in self.cards[1:]]) and set([card.number for card in self.cards]) == {"A", "K", "Q", "J", "T"}

    def is_straight_flush(self):
        if all([self.cards[0].suit == card.suit for card in self.cards[1:]]):
            card_numbers = sorted([card.number_representation for card in self.cards], reverse=True)
            for i in range(1, len(card_numbers)):
                if card_numbers[0] - i != card_numbers[i]:
                    return False
            return True
        return False
    ...
    def is_four_of_a_kind(self):
        four_of_a_kind = None
        not_four_of_a_kind = []
        for (key, value) in Counter([card.number_representation for card in self.cards]).items():
            if value == 4:
                four_of_a_kind = key
            else:
                not_four_of_a_kind.append(key)
        if four_of_a_kind is not None:
            return True, four_of_a_kind, not_four_of_a_kind[0]
        return False
    ...
    def is_full_house(self):
        ...
        if three_of_a_kind is not None and two_of_a_kind is not None:
            return True, three_of_a_kind, two_of_a_kind
        return False
    ...
    def is_three_of_a_kind(self):
        ...
        for (key, value) in Counter([card.number_representation for card in self.cards]).items():
            if value == 3:
                ...
                not_three_of_a_kind.append(key)
        if three_of_a_kind is not None:
            return True, three_of_a_kind, sorted(not_three_of_a_kind, reverse=True)
        return False
    ...
    def is_two_pairs(self):
        ...
        if len(pairs) == 2:
            ...
            return True, pairs[0], pairs[1], not_pairs
        return False
    ...
    def is_pair(self):
        ...
        for (key, value) in Counter([card.number_representation for card in self.cards]).items():
            if value == 2:
                ...
                not_pairs.append(key)
        if pair is not None:
            return True, pair, sorted(not_pairs, reverse=True)
        return False

    def is_winner(self, other):
        my_score = self.score()
        their_score = other.score()
        if my_score[0] == their_score[0]:
            if my_score[0] == 1:
                for i in range(len(my_score[1])):
                    if my_score[1][i] != their_score[1][i]:
                        return my_score[1][i] > their_score[1][i]
            if my_score[0] == 2:
                if my_score[1] != their_score[1]:
                    return my_score[1] > their_score[1]
                for i in range(len(my_score[2])):
                    if my_score[2][i] != their_score[2][i]:
                        return my_score[2][i] > their_score[2][i]
            return None
        return my_score[0] > their_score[0]


class Card:

    def __init__(self, card_string):
        self.number = card_string[0]
        self.number_representation = 14 if self.number == "A" else 13 if self.number == "K" else 12 if self.number == "Q" else 11 if self.number == "J" else 10 if self.number == "T" else int(self.number)
        self.suit = card_string[1]



player_1_wins_count = 0
for hand in hands:
    player_1_hand = Hand(hand[:5])
    player_2_hand = Hand(hand[5:])
    if player_1_hand.is_winner(player_2_hand):
        player_1_wins_count += 1
print(player_1_wins_count)